/* diskio_2.c -- Disk/sector I/O for fst
                     ...
                     int for_write)
{
  ...
  if (sector_size != 0 && sector_size != 512)
    error ("Unsupported sector size %u", (unsigned)sector_size);
...
{
  if (sector_size != 512)
    error ("sector size %u not supported", (unsigned)sector_size);
}
...
/* Write COUNT sectors starting at SEC to HF. */
...
static int write_sec_snapshot (DISKIO *d, const void *src, ULONG sec)
{
  ...
  return write_sec_hfile (d->x.snapshot.hf, FALSE, raw, j, 1);
}
  ...
  switch (d->type)
    ...
    case DIOT_DISK_DASD:
      return write_sec_hfile (d->x.dasd.hf, d->x.dasd.sec_mode, src, sec,
                              count);
    case DIOT_DISK_TRACK:
      return write_sec_track (d->x.track.hf, &d->x.track, src, sec, count);
    case DIOT_SNAPSHOT:
      {
